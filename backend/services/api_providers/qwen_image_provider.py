...
class QwenImageProvider(BaseImageProvider):
    ...
    def generate_image(self, prompt: str, ref_image: Optional[Image.Image] = None, 
        ...
        try:
            # 调用通义千问图像生成
            image_url = self.qwen.generate_image(prompt, **kwargs)
            ...
            logger.error(f"Qwen image generation failed: {str(e)}")
            raise ProviderAPIError(f"Qwen image generation failed: {str(e)}") from e
    ...
    def edit_image(self, prompt: str, current_image: Image.Image,
        ...
        try:
            # 调用通义千问图像编辑
            image_url = self.qwen.edit_image(prompt, current_image, **kwargs)
            ...
            logger.error(f"Qwen image edit failed: {str(e)}")
            raise ProviderAPIError(f"Qwen image edit failed: {str(e)}") from e